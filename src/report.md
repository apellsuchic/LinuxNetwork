...
- ssh adm-ws22@10.20.0.16 -L 8080:localhost:80
  - __Важно отметить, что при данном типе подключения, мы являемя ssh-клиентом и подключаемся к ssh-серверу, на котором по совместительству висит нужный нам веб-сервер__
...
- ssh adm-ws22@10.10.0.50 -R 8080:localhost:80
  ...
  - Здесь практически всё аналогично Local TCP forwarding, но главное отличие кроется в том, что здесь мы предаставляем доступ к веб-серверу на своём хосте, а не получаем его.
